      ...
      {workflows.length === 0 ? (
        ...
        <div className="grid gap-4">
          {workflows.map((workflow) => (
            <Panel key={workflow.id} className="transition hover:border-amber-300/40">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-3 text-xs uppercase tracking-wide text-amber-300">Trigger: {workflow.trigger}</p>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <span className={workflow.enabled ? "text-emerald-300" : "text-rose-300"}>{workflow.enabled ? "Active" : "Paused"}</span>
                  <Link
                    href={`/dashboard/workflows/${workflow.id}`}
                    className="rounded-lg border border-white/20 px-3 py-2 text-slate-100 transition hover:border-amber-300 hover:text-amber-200"
                  >
                    View Details
                  </Link>
                </div>
